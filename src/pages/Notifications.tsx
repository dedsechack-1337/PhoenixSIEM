    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-[#475569] mt-0.5">Configure Email & Slack channels for real-time alert delivery</p>
        </div>
        ...
        <Card className="border-orange-500/30">
          ...
          <CardBody>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#475569] mb-1.5 uppercase tracking-wider">Channel Type</label>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-medium transition-colors ${
                        newChannel.type === t
                          ...
                          : 'bg-[#050d1a] border-[#1a3050] text-[#94a3b8] hover:text-white'
                      }`}
                    >
              ...
              <div>
                <label className="block text-xs text-[#475569] mb-1.5 uppercase tracking-wider">Name</label>
                <input
                  type="text"
                  placeholder="e.g. SOC Team Email"
                  value={newChannel.name || ''}
                  onChange={(e) => setNewChannel((p) => ({ ...p, name: e.target.value }))}
                  className="w-full px-3 py-2 bg-[#050d1a] border border-[#1a3050] rounded-lg text-xs text-white placeholder-[#475569] focus:outline-none focus:border-orange-500/50"
                />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs text-[#475569] mb-1.5 uppercase tracking-wider">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-[#050d1a] border border-[#1a3050] rounded-lg text-xs text-white placeholder-[#475569] focus:outline-none focus:border-orange-500/50 font-mono"
                />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs text-[#475569] mb-1.5 uppercase tracking-wider">Notify on Severity</label>
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg border text-xs transition-colors ${
                        (newChannel.severities || []).includes(sev)
                          ? 'bg-orange-500/20 border-orange-500/40 text-orange-400'
                          : 'bg-[#050d1a] border-[#1a3050] text-[#475569] hover:text-white'
                      }`}
                    >
                      {sev}
                    </button>
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={() => setShowAddForm(false)} className="px-4 py-2 bg-[#050d1a] border border-[#1a3050] text-[#94a3b8] rounded-lg text-xs hover:text-white transition-colors">
                Cancel
              </button>
            </div>
          </CardBody>
      ...
      <Card>
        ...
        <div className="divide-y divide-[#1a3050]">
          {channels.map((ch) => {
            ...
            return (
              <div key={ch.id} className="px-5 py-4 hover:bg-[#0a1628] transition-colors">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded border font-mono ${ch.enabled ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-[#1a3050]/50 border-[#1a3050] text-[#475569]'}`}>
                          {ch.enabled ? 'ACTIVE' : 'DISABLED'}
                        </span>
                      </div>
                      <div className="text-xs text-[#475569] font-mono mt-0.5">{ch.destination}</div>
                      {ch.lastSent && <div className="text-[10px] text-[#475569] mt-0.5">Last sent: {ch.lastSent}</div>}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={ts === 'sending'}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#1a3050] bg-[#0d1f35] text-xs text-[#94a3b8] hover:text-white transition-colors disabled:opacity-50"
                    >
                      {ts === 'sending' ? (
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg border text-xs transition-colors ${
                        ch.enabled
                          ...
                          : 'bg-[#0d1f35] border-[#1a3050] text-[#475569] hover:text-white'
                      }`}
                    >
                      {ch.enabled ? 'Disable' : 'Enable'}
                    </button>
                    <button onClick={() => deleteChannel(ch.id)} className="p-1.5 rounded-lg text-[#475569] hover:text-red-400 hover:bg-red-500/10 transition-colors">
                ...
                <div className="flex items-center gap-2 mt-3 flex-wrap">
                  <span className="text-[10px] text-[#475569] uppercase tracking-wider">Notify on:</span>
                  {SEVERITIES.map((sev) => (
                    <button
                      key={sev}
                      ...
                      className={`px-2 py-0.5 rounded border text-[10px] transition-colors ${
                        ch.severities.includes(sev)
                          ...
                          : 'border-[#1a3050] text-[#475569] hover:text-white'
                      }`}
                    >
                      {sev}
                    </button>
          ...
          {openAlerts.slice(0, 5).map((a) => (
            <div key={a.id} className="flex items-center gap-3 px-5 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-[#475569] font-mono">{a.mitreId} · {a.affectedHosts.join(', ')}</div>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-[#475569] mt-0.5">Recent delivery history</p>
        </CardHeader>
        <div className="divide-y divide-[#1a3050]">
          {logs.map((log) => (
            <div key={log.id} className="flex items-center gap-3 px-5 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-[#475569]">{log.channel} · {log.sentAt}</div>
              </div>
